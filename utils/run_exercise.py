...
class ExerciseRunner:
    ...
    def create_network(self):
        ...
        self.topo = ExerciseTopo(self.hosts, list(self.switches.keys()), self.links, self.log_dir)
    ...
    def auto_arp_tables(self):
        self.logger('auto create arp tables.')
        for host_name in self.topo.hosts():
            h1 = self.net.get(host_name)
            h1_iface = list(h1.intfs.values())[0]
            h1_ip = h1_iface.ip
            h1_mac = h1_iface.mac
            for h2_name in self.topo.hosts():
                if host_name == h2_name:
                    continue
                h2 = self.net.get(h2_name)
                self.logger(h1_ip, h1_mac)
                # h2.setARP(h1_ip, h1_mac)
                h2.cmd('arp -s %s %s -i %s' % (h1_ip, h1_mac, h2_name + '-eth0'))
        pass
    
    def auto_send_probes(self):
        self.logger('auto send probes.')
        for host_name in self.topo.hosts():
            self.logger('%s send probes' % host_name)
            h1 = self.net.get(host_name)
            h1.cmd('python3 test-scripts/probe.py')
        pass

    def run_controller(self):
        self.logger('run controller.')
...
def get_args():
    ...
    parser.add_argument('-j', '--switch_json', type=str, required=False, default = "./build/switch.json")
